...
backup_share() {
        ...
        # Монтування SMB-шари
        sudo mount -t cifs "//$SERVER_IP/$share" "$MOUNT_POINT" -o guest,iocharset=utf8,ro  # Замість guest вказати логін та пароль користувача у форматі: username=<логін>,password=<пароль>
        ...
        # Синхронізація даних
        ##########################################################################
        # Додавання виключень параметром: --exclude='<папка/ або шлях/до/файлу>' #
        # Видалення виключень параметром: --delete-excluded                      #
